...
QString builtinToolDescription(const QString& typeName) {
    static const QMap<QString, QString> kDescriptions = {
        ...
        { "BranchEnd",          "每路分支末尾放置并填写所属分支号, 执行完该分支即跳回公共路径；与选择分支配对" },
        { "ExecuteFlow",        "按名调用另一个流程作为子流程执行, 结果写入当前上下文；用于流程复用与模块化" },
        { "ScriptNode",         "执行JavaScript脚本访问上下文数据与全局变量, 返回值决定OK/NG；用于灵活逻辑与批量计算" },
        { "SwitchImage",        "按条件在多个命名图像间切换并写为当前图像；用于多路图像源选择" },
