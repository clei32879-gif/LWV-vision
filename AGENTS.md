...
| `docs/CKVision资料分析.md` | 原厂手册/真机流程蓝本/现场避坑（references/ckvision_docs/ 有提取文本） |
...
6. **版权姿态**：本项目现阶段为学习研究用途，不必因商用许可限制放弃参考资源；但仍需在代码/文档中注明来源，保留一份"借鉴来源清单"以便将来若商用时评估
7. **测试资产**：检测类改动要用标准图片跑通验证；图片库放 `testdata/`（不入库大文件，入库说明文档）
8. **提交后推送**：每完成一个提交块，`git push origin main` 同步到 GitHub 远端
